...
while(cap.isOpened()):
    ...
    cv2.rectangle(frame, (int(center_x - w/2), int(center_y - h/2)), (int(center_x + w/2), int(center_y + h/2)), (0, 0, 255))
    ...
    if t == 0:
        cv2.imwrite(os.path.join('output', 'ps6-1-a-1.png'), template)
    if (t == 28):
        cv2.imwrite(os.path.join('output', 'ps6-1-a-2.png'), frame)
    if (t == 84):
        cv2.imwrite(os.path.join('output', 'ps6-1-a-3.png'), frame)
    if (t == 144):
        cv2.imwrite(os.path.join('output', 'ps6-1-a-4.png'), frame)
    
    '''
    if (t == 14):
        cv2.imwrite(os.path.join('output', 'ps6-1-e-1.png'), frame)
    if (t == 32):
        cv2.imwrite(os.path.join('output', 'ps6-1-e-2.png'), frame)
    if (t == 46):
        cv2.imwrite(os.path.join('output', 'ps6-1-e-3.png'), frame)
    '''
